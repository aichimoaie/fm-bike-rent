...
- Language switcher in the header (`.lang-switcher`) is currently **cosmetic only** — it toggles the displayed flag/code and matches the reference's dropdown UI/CSS, but does not translate page content (no i18n system exists). Full translation would be a separate, much larger feature.
